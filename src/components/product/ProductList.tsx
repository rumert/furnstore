...
export default function ProductList ({ products }: any) { 
    ...
    <div className='grid grid-cols-5 p-10'>
        ...
        <Link href={`/product/${product.id}`} className='h-fit p-2 border-2 border-transparent hover:border-action-color hover:rounded-xl hover:bg-black hover:bg-opacity-10 rounded-xl' key={index}>
            <Image 
